...
func runService(ctx context.Context, settings config.Config, explicitToken string, args []string, streams IO) int {
	...
	api, source, err := authenticatedServiceClient(ctx, settings, explicitToken)
	if err != nil {
	...
	switch args[0] {
	case "exec":
		return serviceExec(ctx, api, settings, args[1:], streams)
	case "build":
		return serviceBuild(ctx, api, settings, args[1:], streams)
	case "status":
	...
	case "list":
		return serviceList(ctx, api, settings, args[1:], streams)
	case "doctor":
		return serviceDoctor(ctx, api, settings, streams)
	case "token":
		return serviceToken(ctx, api, args[1:], streams)
	case "trust":
		return serviceTrust(ctx, api, settings, args[1:], streams)
	case "admin":
...
func authenticatedServiceClient(ctx context.Context, settings config.Config, explicitToken string) (rtestv1connect.ControlServiceClient, authclient.Source, error) {
	...
	oidc := func(ctx context.Context) (string, error) {
		...
		if settings.Service.Project == "" {
			return "", errors.New("service.project is required for GitHub OIDC")
		}
		...
		response, err := unauthenticated.ExchangeGitHubOIDC(ctx, connect.NewRequest(&rtestv1.ExchangeGitHubOIDCRequest{Project: settings.Service.Project, IdToken: idToken}))
		if err != nil {
			return "", err
		}
...
func serviceExec(ctx context.Context, api rtestv1connect.ControlServiceClient, settings config.Config, args []string, streams IO) int {
	options, err := parseExec(settings, args)
	if err != nil {
...
func parseExec(settings config.Config, args []string) (execOptions, error) {
	options := execOptions{
		project: settings.Service.Project, image: settings.Service.Image, cpus: settings.Service.CPUs,
	...
	if options.project == "" || options.image == "" {
		return execOptions{}, errors.New("project and image are required through configuration or flags")
	}
	return options, nil
}
...
func serviceBuild(ctx context.Context, api rtestv1connect.ControlServiceClient, settings config.Config, args []string, streams IO) int {
	project := settings.Service.Project
	...
	if project == "" {
		return failUsage(streams.Stderr, "build requires a project through configuration or --project")
	}
	if len(args) == 0 {
		args = []string{"."}
	}
...
func serviceList(ctx context.Context, api rtestv1connect.ControlServiceClient, settings config.Config, args []string, streams IO) int {
	project, limit, jsonOutput := settings.Service.Project, 20, false
	for len(args) > 0 {
		switch args[0] {
		case "--project":
			if len(args) < 2 {
...
func serviceDoctor(ctx context.Context, api rtestv1connect.ControlServiceClient, settings config.Config, streams IO) int {
...
func serviceTrust(ctx context.Context, api rtestv1connect.ControlServiceClient, settings config.Config, args []string, streams IO) int {
	...
	switch command {
	case "create":
		values := map[string]string{"--project": settings.Service.Project}
		var events []string
		for len(args) > 0 {
			if len(args) < 2 {
				return failUsage(streams.Stderr, args[0]+" requires a value")
