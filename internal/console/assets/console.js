...
`;var Wh=v0({columnFilteringFeature:Q0,globalFilteringFeature:Y0,filteredRowModel:J0(),filterFns:z0}),Yh=[{id:"search",accessorFn:(x)=>[x.id,x.command,x.projectName,x.project,x.status,x.kind].join(" "),enableGlobalFilter:!0},{id:"status",accessorFn:(x)=>x.status,filterFn:"equalsString",enableGlobalFilter:!1},{id:"kind",accessorFn:(x)=>x.kind,filterFn:"equalsString",enableGlobalFilter:!1}],Jh={now:""};class gd extends Z1(f){static styles=Ud;tableController=new E0(this);query="";statusFilter="";kindFilter="";rowsFingerprint="";rowsCache=[];render(){let x=this.signal("operations",[]),d=this.signal("queue",[]),h=this.signal("clock",Jh),p=this.rows(x,d),i=[...this.statusFilter?[{id:"status",value:this.statusFilter}]:[],...this.kindFilter?[{id:"kind",value:this.kindFilter}]:[]],r=this.tableController.table({features:Wh,columns:Yh,data:p,getCoreRowModel:N1(),globalFilterFn:"includesString",getColumnCanGlobalFilter:(N)=>N.id==="search",state:{globalFilter:this.query,columnFilters:i}}).getRowModel().rows.map((N)=>N.original),k=Date.parse(h.now);return X`
    ...
    `}rows(x,d){let h=JSON.stringify([x,d]);if(h!==this.rowsFingerprint)this.rowsFingerprint=h,this.rowsCache=Bd(x,d);return this.rowsCache}row(x,d){return X`<tr>
      ...
      <div class="sidebar-foot"><div class="identity"><span class="avatar">${gh(x.session.user)}</span><div>
        <div class="identity-name">${x.session.user||"Connecting"}</div><div class="identity-role">${x.session.admin?"Administrator":"Member"}</div>
      </div></div></div>
    </aside>`}navLink(x,d,h,p){return X`<a class="nav-link ${this.routeKind===d?"active":""}" href=${x}>${G(p)}<span>${h}</span></a>`}topbar(x){let d=this.routeKind==="project"?this.project:this.routeKind==="operation"?R(this.operationID,18):this.routeKind==="audit"?"Audit log":"Runs";return X`<header class="topbar">
    ...
    `}resourceMetrics(x){return X`<section class="metrics" aria-label="Runner capacity summary">
      ${F("Busy",D(x.busyRatio),"of the selected hour","pulse")}
      ${F("CPU while active",D(x.cpuAverage),`${D(x.cpuPeak)} peak`,"cpu")}
      ${F("Memory while active",D(x.memoryAverage),`${o(x.memoryBytesPeak)} peak`,"memory")}
      ${F("Queue wait p95",e(x.queueWaitP95Millis),"recent runs","queue")}
    </section>`}resourceChart(x,d){let h=jd(x.samples,(r)=>r.cpuUtilization),p=jd(x.samples,(r)=>r.memoryUtilization),i=x.samples.at(0),v=x.samples.at(-1);return X`<article class="panel resource-panel">
      ...
      ${x.samples.length<2?Q1("activity","Collecting runner data","Utilization will appear after the next samples arrive."):X`
        <div class="chart-legend">
          <span class="legend cpu"><i></i>CPU <strong>${D(x.cpuAverage)} avg · ${D(x.cpuPeak)} peak</strong></span>
          <span class="legend memory"><i></i>Memory <strong>${D(x.memoryAverage)} avg · ${D(x.memoryPeak)} peak</strong></span>
        </div>
        <div class="resource-chart">
          <svg viewBox="0 0 900 230" preserveAspectRatio="none" role="img" aria-label="CPU and memory utilization over time">
            ${[0,0.25,0.5,0.75,1].map((r)=>j`<line class="grid-line" x1="42" y1=${V1(r)} x2="892" y2=${V1(r)}></line><text class="axis-label" x="4" y=${V1(r)+4}>${Math.round(r*100)}%</text>`)}
            <polyline class="series memory" points=${p}></polyline>
            <polyline class="series cpu" points=${h}></polyline>
          </svg>
          <div class="chart-times"><span>${Md(i?.observedAt)}</span><span>${Md(v?.observedAt)}</span></div>
        </div>
      `}
    ...
    </section>`}runSummaryPanel(x,d){return X`<article class="panel"><header class="panel-head"><div class="panel-title">${G("activity")}Run summary</div><span class="panel-meta">${x.resources.sampleCount} samples</span></header>
      <dl class="definition"><dt>Started</dt><dd>${x.startedAt?z1(x.startedAt,d):"—"}</dd><dt>CPU peak</dt><dd>${D(x.resources.cpuPeak)}</dd><dt>Memory peak</dt><dd>${o(x.resources.memoryBytesPeak)}</dd><dt>Queue wait</dt><dd>${e(x.queueWaitMillis)}</dd></dl>
    ...
    </article>`}provenancePanel(x){let d=x.caches?.length?x.caches.map((h)=>h.name).join(", "):"None declared";return X`<article class="panel"><header class="panel-head"><div class="panel-title">${G("fingerprint")}Provenance</div><span class="panel-meta">Inputs</span></header>
    ...
    </table></div>`}pageHead(x,d,h){return X`<header class="page-head"><div><p class="eyebrow">${x}</p><h1>${d}</h1><p class="lede">${h}</p></div><div class="read-only">${G("eye")}CLI-managed</div></header>`}notFound(x){return X`${this.pageHead("Not found","Unavailable",x)}<article class="panel">${Q1("shield","Nothing to show","Return to the console overview.")}</article>`}}function G(x){let d={activity:j`<path d="M3 12h4l2.2-6 4.2 12 2.2-6H21"/>`,clock:j`<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/>`,cpu:j`<rect x="7" y="7" width="10" height="10" rx="2"/><path d="M9 1v3M15 1v3M9 20v3M15 20v3M20 9h3M20 14h3M1 9h3M1 14h3"/>`,cube:j`<path d="m12 3 8 4.5v9L12 21l-8-4.5v-9Z"/><path d="m4.5 7.8 7.5 4.3 7.5-4.3M12 12v9"/>`,disk:j`<ellipse cx="12" cy="5" rx="8" ry="3"/><path d="M4 5v7c0 1.7 3.6 3 8 3s8-1.3 8-3V5M4 12v7c0 1.7 3.6 3 8 3s8-1.3 8-3v-7"/>`,eye:j`<path d="M2 12s3.6-6 10-6 10 6 10 6-3.6 6-10 6S2 12 2 12Z"/><circle cx="12" cy="12" r="2.5"/>`,fingerprint:j`<path d="M8 11a4 4 0 0 1 8 0c0 5-1 8-3 10M5 11a7 7 0 0 1 14 0c0 4-.5 7-2 10M11 14c0 3-.5 5-1.5 7M8 15c0 2-.4 3.5-1 5M12 2a9 9 0 0 0-9 9"/>`,memory:j`<rect x="5" y="7" width="14" height="10" rx="2"/><path d="M8 3v4M12 3v4M16 3v4M8 17v4M12 17v4M16 17v4M9 11h6"/>`,pulse:j`<path d="M3 12h4l2-5 4 10 2-5h6"/>`,queue:j`<path d="M9 6h12M9 12h12M9 18h12"/><circle cx="4" cy="6" r="1"/><circle cx="4" cy="12" r="1"/><circle cx="4" cy="18" r="1"/>`,shield:j`<path d="M12 3 20 6v6c0 5-3.4 8-8 10-4.6-2-8-5-8-10V6Z"/><path d="m9 12 2 2 4-5"/>`,terminal:j`<rect x="3" y="4" width="18" height="16" rx="2"/><path d="m7 9 3 3-3 3M13 15h4"/>`,trend:j`<path d="m3 17 6-6 4 4 8-9"/><path d="M15 6h6v6"/>`};return j`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${d[x]}</svg>`}function F(x,d,h,p){return X`<article class="metric"><div class="metric-top"><span>${x}</span>${G(p)}</div><div class="metric-value">${f1(d)}</div><div class="metric-note">${h}</div></article>`}function Q1(x,d,h){return X`<div class="empty"><div>${G(x)}<strong>${d}</strong><span>${h}</span></div></div>`}function Kh(x){return x.cpuCores?`${x.cpuCores} vCPU · ${o(x.memoryTotalBytes)} · ${o(x.diskTotalBytes)} disk`:"Waiting for capacity data"}function V1(x){return 216-Math.max(0,Math.min(1,x))*196}function jd(x,d){if(x.length===0)return"";return x.map((h,p)=>`${42+p/Math.max(1,x.length-1)*850},${V1(d(h))}`).join(" ")}function Md(x){if(!x)return"—";return new Intl.DateTimeFormat(void 0,{hour:"2-digit",minute:"2-digit"}).format(new Date(x))}function Bh(x){let d=x.filter((h)=>h!=null&&Number.isFinite(h)).sort((h,p)=>h-p);return d.length?d[Math.ceil(d.length*0.95)-1]:void 0}function Uh(x){let d=Object.entries(x.metadata??{}).slice(0,3);return d.length===0?B:X`<div class="metadata">${d.map(([h,p])=>X`<span>${h}=${R(p,28)}</span>`)}</div>`}function f1(x){return x?x[0].toUpperCase()+x.slice(1):"—"}function gh(x){return x.split(/\s+/).filter(Boolean).slice(0,2).map((d)=>d[0]?.toUpperCase()).join("")||"A"}customElements.define("autback-console",nd);
