...
`;function f(e,a=12){if(e.length<=a)return e;return`${e.slice(0,a-1)}…`}function ie(e){if(!e)return"Not configured";let a=e.includes("@")?e.split("@").at(-1):e;return a.length>23?`${a.slice(0,16)}…${a.slice(-6)}`:a}function F(e,a=Date.now()){let t=Date.parse(e);if(!Number.isFinite(t))return"—";let r=Math.max(0,Math.round((a-t)/1000));if(r<5)return"now";if(r<60)return`${r}s ago`;let i=Math.floor(r/60);if(i<60)return`${i}m ago`;let o=Math.floor(i/60);if(o<24)return`${o}h ago`;return`${Math.floor(o/24)}d ago`}function ne(e,a,t=Date.now()){if(!e)return"—";let r=Date.parse(e),i=a?Date.parse(a):t;if(!Number.isFinite(r)||!Number.isFinite(i)||i<r)return"—";let o=i-r;if(o<1000)return`${o}ms`;let n=o/1000;if(n<60)return`${n.toFixed(n<10?1:0)}s`;return`${Math.floor(n/60)}m ${Math.floor(n%60)}s`}function Ae(e){let a=e.filter((r)=>["succeeded","success","failed","cancelled"].includes(r));if(a.length===0)return"—";let t=a.filter((r)=>r==="succeeded"||r==="success").length;return`${Math.round(t/a.length*100)}%`}function w(e){if(!Number.isFinite(e)||e<=0)return"—";let a=["B","KB","MB","GB","TB"],t=Math.min(Math.floor(Math.log(e)/Math.log(1024)),a.length-1),r=e/1024**t;return`${r>=10||Number.isInteger(r)?r.toFixed(0):r.toFixed(1)} ${a[t]}`}function h(e){if(!Number.isFinite(e))return"—";return`${Math.round(Math.max(0,Math.min(1,e))*100)}%`}function q(e){if(e==null||!Number.isFinite(e)||e<0)return"—";if(e<1000)return`${Math.round(e)}ms`;let a=e/1000;if(a<60)return`${a.toFixed(a<10?1:0)}s`;return`${Math.floor(a/60)}m ${Math.floor(a%60)}s`}var _e={samples:[],sampleCount:0,activeSampleCount:0,cpuCores:0,memoryTotalBytes:0,diskUsageBytes:0,diskTotalBytes:0,busyRatio:0,cpuAverage:0,cpuPeak:0,memoryAverage:0,memoryPeak:0,memoryBytesPeak:0,queueWaitP95Millis:0},x={session:{user:"",admin:!1,projects:[]},service:{name:"Autback",version:"",control:"CLI only",admission:"One at a time",startedAt:""},worker:{status:"connecting",capacity:"1 operation",activeId:"",updatedAt:""},resources:_e,queue:[],operations:[],operation:null,log:{available:!1,truncated:!1,content:""},audit:[],status:{ready:!1,route:"",message:"Connecting",updatedAt:""}};class Ue extends De(j){static styles=qe;get routeKind(){return this.getAttribute("route-kind")||"overview"}get project(){return this.getAttribute("project")||""}get operationID(){return this.getAttribute("operation-id")||""}render(){let e=this.signals();return s`<div class="shell">
    ...
    </div>`}signals(){return{session:this.signal("session",x.session),service:this.signal("service",x.service),worker:this.signal("worker",x.worker),resources:this.signal("resources",x.resources),queue:this.signal("queue",x.queue),operations:this.signal("operations",x.operations),operation:this.signal("operation",x.operation),log:this.signal("log",x.log),audit:this.signal("audit",x.audit),status:this.signal("status",x.status)}}sidebar(e){return s`<aside class="sidebar" aria-label="Console navigation">
    ...
    </header>`}page(e){switch(this.routeKind){case"project":return this.projectPage(e);case"operation":return this.runPage(e);case"audit":return this.auditPage(e);default:return this.overview(e)}}overview(e){return s`
      ...
      <section class="grid">
        ${this.jobsPanel(e.queue)}
        ${this.runnerPanel(e)}
      </section>
      ${this.runsPanel(e.operations,"Recent runs")}
    `}projectPage(e){let a=e.session.projects.find((t)=>t.slug===this.project);if(!a)return this.notFound("You do not have access to this project.");return s`
      ${this.pageHead("Project",a.name,"Runs, demand, and runner use for this project.")}
      <section class="project-banner">
        <div><div class="project-name">${a.name}</div><div class="project-slug">${a.slug}</div><p class="digest">${ie(a.activeImage)}</p></div>
        <div class="project-facts">
          ...
          <div class="fact"><strong>${a.allowImageOverrides?"Flexible":"Pinned"}</strong><span>Runner image</span></div>
        </div>
      </section>
      ${this.projectTrends(e.operations)}
      ${this.resourceChart(e.resources,"Resource utilization")}
      <section class="grid">${this.jobsPanel(e.queue)}${this.runnerPanel(e)}</section>
      ${this.runsPanel(e.operations,"Project runs")}
    `}runPage(e){let a=e.operation;if(!a)return this.notFound("You do not have access to this run.");let t=a.command||`${O(a.kind)} ${f(a.id,18)}`;return s`
      ${this.pageHead(`${O(a.kind)} run`,t,`${a.projectName} · ${f(a.id,26)}`)}
      ${this.resourceChart(e.resources,"Resource utilization")}
      <section class="metrics" aria-label="Run summary">
        ${$("Status",a.status,O(a.kind),"pulse")}
        ${$("Queue wait",q(a.queueWaitMillis),"before starting","queue")}
        ${$("Duration",ne(a.startedAt,a.finishedAt),a.startedAt?"elapsed time":"not started","clock")}
        ${$("Exit code",a.exitCode==null?"—":String(a.exitCode),a.finishedAt?"result":"pending","terminal")}
      </section>
      <section class="detail-grid">
        <div class="detail-stack">
          <article class="panel"><header class="panel-head"><div class="panel-title">${u("terminal")}Command</div><span class="badge ${a.status}">${a.status}</span></header>
            <div class="panel-body"><pre class="command"><span class="prompt">$</span> ${a.command||"docker buildx build"}</pre></div>
          </article>
          ${this.logPanel(e,a)}
        </div>
        <div class="detail-stack">${this.runSummaryPanel(a)}${this.provenancePanel(a)}
          <article class="panel"><header class="panel-head"><div class="panel-title">${u("terminal")}Continue in CLI</div><span class="panel-meta">CLI</span></header>
            <div class="panel-body"><p class="lede">View the full log or inspect this run from your terminal.</p><pre class="command"><span class="prompt">$</span> autback ${a.kind==="job"?"logs":"build status"} ${a.id}</pre></div>
          </article>
        </div>
      </section>
    `}resourceMetrics(e){return s`<section class="metrics" aria-label="Runner capacity summary">
    ...
    </section>`}resourceChart(e,a){let t=Ve(e.samples,(n)=>n.cpuUtilization),r=Ve(e.samples,(n)=>n.memoryUtilization),i=e.samples.at(0),o=e.samples.at(-1);return s`<article class="panel resource-panel">
      ...
      ${e.samples.length<2?T("activity","Collecting runner data","Utilization will appear after the next samples arrive."):s`
        ...
        <div class="resource-chart">
          <svg viewBox="0 0 900 230" preserveAspectRatio="none" role="img" aria-label="CPU and memory utilization over time">
            ${[0,0.25,0.5,0.75,1].map((n)=>m`<line class="grid-line" x1="42" y1=${G(n)} x2="892" y2=${G(n)}></line><text class="axis-label" x="4" y=${G(n)+4}>${Math.round(n*100)}%</text>`)}
            <polyline class="series memory" points=${r}></polyline>
            <polyline class="series cpu" points=${t}></polyline>
          </svg>
          <div class="chart-times"><span>${Ie(i?.observedAt)}</span><span>${Ie(o?.observedAt)}</span></div>
        </div>
      `}
    </article>`}projectTrends(e){let a=e.filter((i)=>i.startedAt&&i.finishedAt).slice(0,20).reverse(),t=a.map((i)=>Date.parse(i.finishedAt)-Date.parse(i.startedAt)),r=Math.max(...t,1);return s`<section class="trend-grid">
      <article class="panel trend-panel"><header class="panel-head"><div class="panel-title">${u("clock")}Run duration</div><span class="panel-meta">Last ${a.length}</span></header>
        <div class="duration-bars">${t.length===0?T("clock","No completed runs","Duration history will appear here."):t.map((i)=>s`<i style=${`height:${Math.max(5,i/r*100)}%`} title=${q(i)}></i>`)}</div>
      </article>
      <article class="panel project-health"><div><span>Success rate</span><strong>${Ae(e.map((i)=>i.status))}</strong></div><div><span>Queue wait p95</span><strong>${q(aa(e.map((i)=>i.queueWaitMillis)))}</strong></div></article>
    </section>`}jobsPanel(e){return s`<article class="panel">
      <header class="panel-head"><div class="panel-title">${u("queue")}Jobs</div><span class="panel-meta">${e.length}</span></header>
      <div class="queue-list">${e.length===0?T("queue","No jobs queued or active","The next submitted job can start immediately."):e.map((a)=>s`
        <div class="queue-row"><span class="position">${a.position}</span>
          <div class="queue-main"><a href=${Se(a.kind,a.id)}>${f(a.id,24)}</a><div class="queue-sub">${a.projectName} · ${F(a.acceptedAt)}</div></div>
    ...
    </article>`}runsPanel(e,a){return s`<article class="panel"><header class="panel-head"><div class="panel-title">${u("activity")}${a}</div><span class="panel-meta">${e.length} shown</span></header>
      ${e.length===0?T("activity","No runs yet","Submit a repository command with autback exec."):s`<div class="table-wrap"><table>
        ...
        <tbody>${e.map((t)=>s`<tr>
          <td class="primary"><a href=${Se(t.kind,t.id)}><span class="kind-icon">${u(t.kind==="build"?"cube":"terminal")}</span><span><span class="mono">${f(t.id,20)}</span><br><span class="muted">${t.command||O(t.kind)}</span></span></a></td>
          <td><span class="badge ${t.status}">${t.status}</span></td><td>${t.projectName}</td>
          <td class="mono">${ne(t.startedAt,t.finishedAt)}</td>
          <td class="mono">${t.resources?.sampleCount?h(t.resources.cpuPeak):"—"}</td>
          <td class="mono">${t.resources?.sampleCount?w(t.resources.memoryBytesPeak):"—"}</td>
          <td>${F(t.createdAt)}</td>
        </tr>`)}</tbody>
      </table></div>`}
    </article>`}runSummaryPanel(e){return s`<article class="panel"><header class="panel-head"><div class="panel-title">${u("activity")}Run summary</div><span class="panel-meta">${e.resources.sampleCount} samples</span></header>
      <dl class="definition"><dt>Started</dt><dd>${e.startedAt?F(e.startedAt):"—"}</dd><dt>CPU peak</dt><dd>${h(e.resources.cpuPeak)}</dd><dt>Memory peak</dt><dd>${w(e.resources.memoryBytesPeak)}</dd><dt>Queue wait</dt><dd>${q(e.queueWaitMillis)}</dd></dl>
      ...
      <dl class="definition"><dt>Run</dt><dd>${e.id}</dd><dt>Project</dt><dd>${e.project}</dd><dt>Image</dt><dd title=${e.image}>${ie(e.image)}</dd><dt>Workdir</dt><dd>${e.workingDirectory||"—"}</dd><dt>Root</dt><dd>${e.rootDigest||"—"}</dd><dt>Caches</dt><dd>${a}</dd></dl>
    </article>`}auditPage(e){return s`${this.pageHead("Governance","Audit log","Project, access, image, job, and build activity across Autback.")}
      ...
      ${e.audit.length===0?T("shield","No audit events yet","Changes made with the Autback CLI will appear here."):this.auditTable(e.audit)}</article>`}auditTable(e){return s`<div class="table-wrap"><table><thead><tr><th>Event</th><th>Actor</th><th>Project</th><th>Target</th><th>When</th></tr></thead>
      <tbody>${e.map((a)=>s`<tr><td><span class="audit-action">${a.action}</span>${ta(a)}</td><td>${a.actor}</td><td>${a.project||"Service"}</td><td class="mono">${f(a.target,18)}</td><td>${F(a.createdAt)}</td></tr>`)}</tbody>
