// Package projectlink resolves the non-secret autback project selected by a Git repository.
package projectlink
...
func Resolve(ctx context.Context, directory, explicit, environment string) (string, error) {
	...
	return "", fmt.Errorf("project is required: pass --project, set AUTBACK_PROJECT, or run autback init to create %s", FileName)
}
...
func Write(directory, project string) (string, error) {
	...
	temporary, err := os.CreateTemp(directory, ".autback-link-*")
	if err != nil {
		return "", err
	}
